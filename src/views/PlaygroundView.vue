...
<template>
  ...
  <div class="text-white" v-if="processVisible">
    ...
    <p text="center">
      <span text="green-400">{{ currentIndex + 1 }}</span
      ><span mx="1">/</span><span>{{ count }}</span>
    </p>
  </div>
  ...
  <div class="flex-center" flex="col">
    ...
    <div class="my-4 flex-center">
      <!-- 上一个 -->
      <div
        class="switch-btn hover:(text-green-400 -translate-x-1)"
        v-if="processVisible"
      >
        <ChevronLeft :size="40" />
      </div>
      ...
      <div class="w-18">
        <input
          ...
          :class="[
            'signle-inputbox',
            !!firstAnswer ? `text_${firstIsRight ? 'right' : 'error'}` : '',
            firstShake ? 'shake' : '',
          ]"
        />
      ...
      <div class="w-18 ml-4" v-if="secondInputVisible">
        <input
          ...
          :class="[
            'signle-inputbox',
            !!secondAnswer ? `text_${secondIsRight ? 'right' : 'error'}` : '',
            secondShake ? 'shake' : '',
          ]"
        />
    ...
    <p text="white" select="none">
      ...
      <strong
        text="green-400"
        cursor="pointer"
        @click="handleTypeSwitch('question')"
        >{{ PATTERN[qTIndex].name }}</strong
      >
      相对应的
      <strong
        text="green-400"
        cursor="pointer"
        @click="handleTypeSwitch('answer')"
...
// 所有的假名
const kanas = [...KANA, ...DULL, ...AO, ...AO_DULL].filter(
  (kana) => !isEmpty(kana)
);
// 当前是第几个
...
// 用于渲染题目的数据
const questionRenderData = computed(() =>
  processVisible.value ? randomKanas.value : kanas
);
// 当前显示的假名题目
const currentKana = computed(
  () => questionRenderData.value[currentIndex.value]
);
// 验证答案是否正确
...
const answerValidation = computed(() =>
  !isFix.value ? firstIsRight.value : firstIsRight.value && secondIsRight.value
);
// 答案类型
...
const currentQuestion = computed(() => {
  const question =
    currentKana.value[PATTERN[qTIndex.value].type as keyof Kana] || "";
...
const inputLabel = computed(
  () => (index: number) => currentAType.value.name.split("/")[index]
);
// 生成输入框 placeholder
const placeholders = computed(() => {
  return currentAType.value.type.split("/").map((type, index) => {
    ...
    if (
      (!index && firstErrorTime.value >= 3) ||
      (index && secondErrorTime.value >= 3)
    ) {
...
const handleNext = () => {
  ...
  if (answerValidation.value) {
    ...
    currentIndex.value = processVisible.value
      ? currentIndex.value + 1
      : getRandomNum(kanas.length);
  } else {
...
const generateLocalData = () => {
  ...
  if (!Object.keys(localData.recordData.data).includes(recordKey)) {
    ...
    localData.recordData.data[recordKey] = {
      ...currentKana.value,
      rightTimes:
        localData.recordData.data[recordKey].rightTimes +
        Number(answerValidation.value),
      errorTimes:
        localData.recordData.data[recordKey].errorTimes +
        Number(!answerValidation.value),
    };
  }
};
// 获取随机数
const getRandomNum = (baseNum: number): number =>
  Math.ceil(Math.random() * (baseNum === kanas.length ? baseNum - 1 : baseNum));
// 监听 enter 按键事件
...
const validation = (val: string, typeIndex = 0) => {
  ...
  if (type === "roma") {
    ...
    return typeof targetVal === "string"
      ? _val === targetVal
      : (targetVal || []).includes(_val);
  }
