...
<template>
  ...
  <template v-if="!showPlayground">
    <div text="center white">
      <div class="flex-center">背他个<Selector @select="handleSelect" :data="exerciseSize" :selected-option="account"></Selector>个单词卷死他们！🤪</div>
      <p
        w="26"
        text="zinc-400"
  ...
  <template v-else>
    ...
    <div text="white">
      <div w="40" mb="1" position="relative" class="p_progress">
        <div bg="green-400" h="2" position="absolute" top="-0.5" left="0" border="rounded" :style="processStyle"></div>
      </div>
      <p text="center">
        <span text="green-400">{{ processCurrent }}</span><span mx="1">/</span><span>{{ account }}</span>
      </p>
    </div>
    ...
    <div class="flex-center" flex="col">
      ...
      <input
        ...
        :style="inputStyle"
        :class="[!!answers[processCurrent - 1] ? `text_${isRight ? 'right' : 'error'}` : '', shake ? 'shake' : '']"
      />

      <p flex="center" text="sm zinc-400 center" select="none">
        音型: {{ currentQuestion.tone }} | 词性: {{ currentQuestion.kind }} | 熟练度:
        <span
          v-for="index in 3"
          class="inline-block"
          h="1"
          ...
          ml="1"
          :key="index"
          :bg="index > (currentQuestion.degree || 0) ? 'gray-500' : 'green-400'"
        ></span>
      </p>
      <!-- TODO 添加例句，试试 chatGpt ？ -->
      <p flex="center" text="sm zinc-400 center" select="none">例句: </p>

      <div mt="4" text="xs gray-500" class="cursor-pointer" v-if="nextVisible" @click="handleNext">
        按下<span text="green-400 xs" border="b-1 b-zinc-500 rounded-sm" mx="2" bg="zinc-700" p="1">ENTER</span>键切换下一个
      </div>
    </div>
...
<script lang="ts" setup>
  ...
  const processStyle = computed(() => isInfinite.value ? {
      width: '100%'
    } : {
      width: `${(processCurrent.value / (account.value as number) * 100).toFixed(2)}%`
    }
  )
  const allWords: ComputedRef<Word[]> = computed(() => Object.entries(WORDS).reduce((arr, [, word]) => [...arr, ...word.words], []))
  const currentQuestionIndex: ComputedRef<number> = computed(() => isInfinite.value ? currentIndex.value : questions.value[processCurrent.value - 1])
  ...
  const isInfinite: ComputedRef<boolean> = computed(() => typeof(account.value) === 'string')
  const nextVisible: ComputedRef<boolean> = computed(() => isInfinite.value || processCurrent.value < questions.value.length)
  ...
  const getRandomNum = () => Math.ceil(Math.random() * (allWords.value.length - 1))
  const handleNext = () => {
    if (showPlayground.value) {
      ...
      answersDegree.value.set(currentQuestion.value.kana, (answersDegree.value.get(currentQuestion.value.kana) || 0) + (!!errorTimes.value ? -1 : 1))
